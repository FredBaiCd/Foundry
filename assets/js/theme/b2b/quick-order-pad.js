...
export default function(customer) {
    ...
    $("body").on("click", '[select-product-item]', function() {
        ...
        utils.api.product.getById(product_id, {
            ...
            if ($qtyInput.val()) {
                $totalPriceContainer.text(`${gPriceSymbol}${pricesStyle(tier_price*qty, 2)}`);
            }
            ...
            utils.api.product.getById(product_id, {
                ...
                const tr = `
                    <tr data-variant-id="${variant_id}"  data-product-id="${product_id}" csv-tr-${i}>
                        ...
                        <td class="col-price"><span data-total-price>${gPriceSymbol}${pricesStyle(tierPrice*product_qty, 2)}</span></td>
                    </tr>`;
