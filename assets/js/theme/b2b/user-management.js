...
export default function(customer) {
	...
	const $newUserModal = $("#modal-user-management-new");
	const $editUserModal = $("#modal-user-management-edit");

	const $newUserModalObj = new Modal({
		el: $('#modal-user-management-new'),
		modalClass: 'modal--small'
	});
	const $editUserModalObj = new Modal({
		el: $('#modal-user-management-edit'),
		modalClass: 'modal--small'
	});

	$('[data-reveal-id="modal-user-management-new"]').on('click', () => {
		$newUserModalObj.open();
	});
	$('body').on('click', '[data-reveal-id="modal-user-management-edit"]', () => {
		$editUserModalObj.open();
	});
						...
						if (gRoleId == 1) {
							tr = `<tr data-user='${JSON.stringify(userdata)}'>
				    			...
				    			<td class="actions-field t-align-r">
				    			    <a href="#" data-reveal-id="modal-user-management-edit" class="button button--primary button--small" data-edit-user>Edit</a>
				    			    <span class="actions-split">|</span>
				    			    <a href="#" data-delete-user class="button button--small button--secondary">Delete</a></td>
				    		</tr>`;
	...
	// close the modal
	$newUserModalObj.$el.find(".modal-close").on('click', function() {
		console.log('close the modal')
		console.log('lo', $newUserModalObj.$modal.find('form').eq(0)[0])
		// reset data
		$newUserModalObj.$modal.find('form').eq(0)[0].reset()
		// reset style
		var cd = $newUserModalObj.$modal.find('.form-fieldset').eq(0).children().removeClass('form-field--error').removeClass('form-field--success')
		let fields = $newUserModalObj.$modal.find('.form-fieldset').eq(0).children()
	...
	let newUserValidator = nod({
		button: '#user_management_new_form input[type="button"]',
	});

	const newUserFormSelector = `#user_management_new_form`;
			...
			success: function(data) {
				...
				if (data) {
					if (data.emailTaken) {
						...
						$newUserModalObj.$modal.find(".modal-close").eq(0).click();
						load_table();
					}
	...
	let updateUserValidator = nod({
		button: '#user_management_edit_form input[type="button"]',
	});

	const updateUserFormSelector = `#user_management_edit_form`;
	...
	$("#update_user").on("click", function() {
		...
		$.ajax({
			...
			success: function(data) {
				console.log(data);
				$editUserModalObj.$modal.find(".modal-close").eq(0).click();
				load_table();
