...
export default function(customer) {
					...
					if (data.length > 0) {
						...
						for (let i = 0; i < listsData.length; i++) {
							...
							if (isOwn && notReadyForApproval && notJuniorApproval) {
								deleteBtn = `<a class="button button--small button--secondary" href="javascript:void(0);" data-delete-list data-list-id="${listData.id}">Delete</a>`;
							} else {
								deleteBtn = `<a class="button button--small button--secondary" href="javascript:void(0);" disabled>Delete</a>`;
							}
	...
	const $newListModalObj = new Modal({
		el: $('#modal-shopping-list-new-form'),
		modalClass: 'modal--small'
	});

	$("#shopping_list_new").on('click', function() {
		$newListModalObj.open();
		$newListModalObj.$modal.find('form').eq(0)[0].reset();
	});
