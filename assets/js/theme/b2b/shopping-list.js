...
export default function(customer) {
	...
	const $renameListModalObj = new Modal({
		el: $('#modal-shopping-list-rename-form'),
...
*/
	...
	**/
	...
	$("[data-rename-list]").on('click', function() {
		$renameListModalObj.open();
		const $form = $renameListModalObj.$modal.find("form");
			...
			if (catalog_products[product_id]) {
				...
				for (let i = 0; i < variants.length; i++) {
					...
					resultTrs += `<tr data-product-id="${product_id}" data-variant-id="${variant_id}">
					        ...
					        <td class="col-product-price">${price_symbol}${pricesStyle(catalog_price, 2)}</td></tr>`;
				}
				...
				utils.api.product.getById(product_id, {
					...
					if (resultTrs) {
						...
						$optionLabels.each(function() {
							...
							const $optionInput = $optionLabel.find(`[name*="attribute["]`);
					...
					debugger
					const $select = $("#product_search_results").find('select');
					if ($select.length) {
						$select.each((i, el) => {
				...
				if (catalog_products[product_id]) {
					...
					utils.api.product.getById(product_id, {
						...
						if (resultTrs) {
							...
							if (catalog_price) {
								$("#skus_search_results").find(`[product-search-result-table] tbody tr[data-product-id="${product_id}"] [data-product-price]`).text("$" + pricesStyle(catalog_price, 2));;
							}
						...
						const $select = $("#skus_search_results").find('select');
						if ($select.length) {
							$select.each((i, el) => {
								new SelectWrapper(el);
							});
			...
			if (itemArr.length > 0) {
				...
				utils.api.cart.getContent(options, (err, response) => {
					...
					if ($items.length > 0) {
						...
						$("#page_bottom_cart_nav .cart-quantity").text(cartQuantity);
		...
		$.ajax({
			...
			success: (data) => {
				...
				if (cartItemIDs.length > 0) { //if there are items in cart notify user
					...
					/*$(".swal2-confirm.btn-cart-merge").after('<button type="button" class="button replace-button">Replace</button>');
					$(".swal2-confirm.btn-cart-merge").on("click", function() {
						$overlay.show();
						addProductToCart(itemArr);
					});
	...
	const listEditOptions = function(productId, variantId, itemIndex, itemOptions, skuHtml) {
		...
		const modal = new Modal({
			el: $('#modal-shopping-list-option-form'),
		...
		utils.api.product.getById(productId, {
			...
			modal.open();
			modal.$modal.find("#index_container").attr("data-index", itemIndex);
			modal.$modal.find("#variant_id_container").attr("data-variant-id", variantId);
			modal.$modal.find("[data-product-sku]").html(skuHtml);
			...
			for (let j = 0; j < options.length; j++) {
				...
				const $option = modal.$modal.find(`[name="${optionName}"]`);
				if ($option.length > 0) {
		...
		utils.api.cart.itemRemove(cartitem.id, (err, response) => {
			if (response.data.status === 'succeed') {
				...
				if (cartItemArr.length > 0) {
					...
					$("#page_bottom_cart_nav .cart-quantity").text(0);
			...
			success: (data) => {
				...
				if (catalog_products && catalog_products[productId]) {
					...
					for (let j = 0; j < tier_price_array.length; j++) {
						...
						const tier_qty = tier_price_array[j].qty;
						const price = tier_price_array[j].price;
						const new_price = tier_price_array[j].new_price || 0;
	...
	// for simple products
	const getTierPriceByProductId_multi = function(tproductIds, qty, cb) {
		const productIds = tproductIds;
	...
	const initProductListOptionPrice = function() {
		...
		const $form = $('form[data-option-form]');
